...
export function createConsultarPedidoActions(page: Page) {
  return {
    // async validarQueEstaNaPagina() {
    async abrirPaginaVelo(){
      await page.goto('/');
      const titulo = page.getByTestId('hero-section').getByRole('heading');
  ...
  - paragraph: /R\\$ \\d+\\.\\d+,\\d+/
`;
      const locator = page.getByTestId(`order-result-${order.number}`);
